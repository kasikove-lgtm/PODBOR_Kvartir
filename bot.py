...
ЕСЛИ ЭТО СКРИНШОТ КАРТЫ с отметкой дома — посмотри визуально, что есть в непосредственной
близости (в пределах примерно 200-300 метров от отметки): железнодорожные пути, трамвайные
линии, крупная многополосная дорога/шоссе. Если что-то из этого видно рядом — запиши в
noise_nearby, например "Рядом трамвайные пути (~150м)" или "Рядом ж/д пути". Если в радиусе
ничего шумного не видно — оставь noise_nearby пустым (не пиши "шума нет", просто пустая строка,
это поле заполняется только при наличии находки). Остальные поля на карте обычно отсутствуют.
