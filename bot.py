...
Как понять назначение комнаты на плане, если рядом с площадью нет текстовой подписи (часто
бывает только номер комнаты и цифра площади, без слов):
- Кухня — узнаётся по значкам плиты и раковины (мойки), нарисованным внутри помещения.
  Площадь этой комнаты пиши в area_kitchen, а не в area_living.
- Санузел/ванная — узнаётся по значкам унитаза, ванны или душевой кабины. Бери оттуда bathroom.
- Прихожая/коридор — узкое помещение у входной двери, обычно без окон и без сантехники,
  часто соединяет комнаты. Площадь — в hallway.
- Балкон/лоджия — отдельный узкий выступ за пределами основного контура квартиры. Площадь — в balcony.
- Все оставшиеся жилые комнаты (без сантехники, обычно с окном, не прихожая) — их площади суммируй
  в area_living. area_total — это сумма площадей вообще всех помещений на плане (включая кухню,
  санузел, прихожую, но обычно БЕЗ балкона — он считается отдельно с пониженным коэффициентом
  или не входит совсем; если не уверен в точной сумме, area_total бери из подписи на самом плане,
  если она там есть, а не считай сам).
Если на плане есть прямые текстовые подписи (например "кухня 9.4", "ванная 3.2") — используй их,
это надёжнее значков.
